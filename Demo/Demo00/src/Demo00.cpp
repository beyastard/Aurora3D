...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char* lpCmdLine, int nCmdShow)
{
	...
	{
		Log(LogLevel::Error, "WinMain, Failed to register window class!");
		return -1;
	}
	...
	{
		Log(LogLevel::Error, "WinMain, Failed to create main window!");
		return -1;
	}
...
ATOM RegisterWndClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON_ZX));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = static_cast<HBRUSH>(GetStockObject(BLACK_BRUSH));
	wcex.lpszMenuName = nullptr;
	wcex.lpszClassName = l_szClassName.c_str();
	wcex.hIconSm = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON_ZX));
...
bool CreateMainWnd(HINSTANCE hInstance, int nCmdShow)
{
	...
	HWND hWnd = CreateWindowEx(dwExStyle, l_szClassName.c_str(), l_szWndName, dwStyles, 0, 0,
