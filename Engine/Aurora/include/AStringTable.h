...
public:
    ...
    bool GetEntry(std::string_view entryName, std::string& entryData) const;

    // Get entry by index
    bool GetEntryDataByIndex(size_t index, std::string& entryData) const;
    bool GetEntryNameByIndex(size_t index, std::string& entryName) const;

    // Add entry (name converted to uppercase)
    bool AddEntry(std::string_view entryName, std::string_view entryData);
