...
public:
	...
	bool Open(std::string_view pckPath, AFPCK_OPENMODE mode);
	bool Close();

	bool AppendFile(std::string_view fileName, std::span<const std::byte> fileData);
	bool RemoveFile(std::string_view fileName);
	bool ReplaceFile(std::string_view fileName, std::span<const std::byte> fileData);

	bool ReadFile(std::string_view fileName, std::span<std::byte> buffer, std::uint32_t offset, std::uint32_t& bytesRead);
	bool ReadFile(const AFPCK_FILEENTRY& entry, std::span<std::byte> buffer, std::uint32_t offset, std::uint32_t& bytesRead);

	bool GetFileEntry(std::string_view fileName, AFPCK_FILEENTRY& outEntry, int* outIndex = nullptr) const;
...
private:
	bool LoadEntries();
	bool SaveEntries();
	std::string NormalizeFileName(std::string_view fileName) const;
