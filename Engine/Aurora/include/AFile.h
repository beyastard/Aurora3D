...
public:
    ...
    // Open with full path
    virtual bool Open(std::string_view fullPath, std::uint32_t flags);

    // Open with folder + filename (resolves via AFI)
    virtual bool Open(std::string_view folderName, std::string_view fileName, std::uint32_t flags);
    ...
    [[nodiscard]] const std::string& GetRelativeName() const noexcept { return m_relativeName; }

protected:
    std::string m_fileName;     // full path
    std::string m_relativeName; // relative to base dir
