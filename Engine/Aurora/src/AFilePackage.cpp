...
bool AFilePackage::Open(std::string_view pckPath, AFPCK_OPENMODE mode)
{
    ...
    if (mode == AFPCK_OPENMODE::AFPCK_CREATENEW)
    {
        ...
        if (!m_packageFile)
        {
            AFERRLOG("AFilePackage::Open(), Can not create file [{}]", pckPath);
            return false;
        }
    ...
    else
    {
        ...
        if (!m_packageFile)
        {
            ...
            if (!m_packageFile)
            {
                AFERRLOG("AFilePackage::Open(), Can not open file [{}]", pckPath);
                return false;
            }
        ...
        {
            AFERRLOG("AFilePackage::Open(), Incorrect version! Got {:#x}", version);
            return false;
        }
...
bool AFilePackage::AppendFile(std::string_view fileName, std::span<const std::byte> fileData)
{
    if (m_readOnly)
    {
        AFERRLOG("AFilePackage::AppendFile(), Read-only package");
        return false;
    }
...
bool AFilePackage::RemoveFile(std::string_view fileName)
{
    if (m_readOnly)
    {
        AFERRLOG("AFilePackage::RemoveFile(), Read-only package");
        return false;
    }
    ...
    {
        AFERRLOG("AFilePackage::RemoveFile(), File not found: {}", fileName);
        return false;
    }
...
bool AFilePackage::ReplaceFile(std::string_view fileName, std::span<const std::byte> fileData)
{
    if (m_readOnly)
    {
        AFERRLOG("AFilePackage::ReplaceFile(), Read-only package");
        return false;
    }
    ...
    if (!GetFileEntry(fileName, entry, &index))
    {
        AFERRLOG("AFilePackage::ReplaceFile(), File not found: {}", fileName);
        return false;
    }
...
bool AFilePackage::ReadFile(std::string_view fileName, std::span<std::byte> buffer, std::uint32_t offset, std::uint32_t& bytesRead)
{
    AFPCK_FILEENTRY entry;
    if (!GetFileEntry(fileName, entry))
    {
        AFERRLOG("AFilePackage::ReadFile(), Can not find file entry [{}]", fileName);
        return false;
    }
...
bool AFilePackage::ReadFile(const AFPCK_FILEENTRY& entry, std::span<std::byte> buffer, std::uint32_t offset, std::uint32_t& bytesRead)
{
    ...
    {
        AFERRLOG("AFilePackage::ReadFile(), Offset [{}] beyond file length [{}]", offset, entry.dwLength);
        return false;
    }
    ...
    {
        AFERRLOG("AFilePackage::ReadFile(), Buffer too small: {} < {}", buffer.size(), bytesToRead);
        return false;
    }
    ...
    if (entry.dwCompressedLength < entry.dwLength)
    {
        if (offset != 0)
        {
            AFERRLOG("AFilePackage::ReadFile(), Offset not allowed for compressed files");
            return false;
        }
        ...
        if (result != Z_OK)
        {
            AFERRLOG("AFilePackage::ReadFile(), Decompression failed: {}", result);
            return false;
        }
...
bool AFilePackage::GetFileEntry(std::string_view fileName, AFPCK_FILEENTRY& outEntry, int* outIndex) const
{
...
bool AFilePackage::LoadEntries()
{
    ...
    {
        AFERRLOG("AFilePackage::LoadEntries(), Invalid file count");
        return false;
    }
    ...
    {
        AFERRLOG("AFilePackage::LoadEntries(), Failed to read header");
        return false;
    }
    ...
    for (int i = 0; i < numFiles; ++i)
    {
        ...
        {
            AFERRLOG("AFilePackage::LoadEntries(), Invalid filename length: {}", nameLen);
            return false;
        }
        ...
        if (m_packageFile.fail())
        {
            AFERRLOG("AFilePackage::LoadEntries(), Failed to read entry {}", i);
            return false;
        }
    }
...
std::string AFilePackage::NormalizeFileName(std::string_view fileName) const
{
    return ::NormalizeFileName(fileName);
}

bool OpenFilePackage(const char* szPackFile)
{
    CloseFilePackage(); // ensure clean state
    g_globalPackage = std::make_unique<AFilePackage>();

    return g_globalPackage->Open(szPackFile, AFPCK_OPENMODE::AFPCK_OPENEXIST);
}
