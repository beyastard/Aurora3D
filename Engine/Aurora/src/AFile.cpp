...
bool AFile::Open(std::string_view fullPath, std::uint32_t flags)
{
    if (m_isOpen)
        Close();
    ...
    if (!m_file)
    {
        AFERRLOG("Failed to open file: {}", m_fileName);
        return false;
    }
...
bool AFile::Open(std::string_view folderName, std::string_view fileName, std::uint32_t flags)
{
