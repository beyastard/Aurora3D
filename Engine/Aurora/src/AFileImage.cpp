...
bool AFileImage::Open(std::string_view folderName, std::string_view fileName, unsigned int flags)
{
    std::string fullPath = AFileMod::GetFullPath(folderName, fileName);
    return Open(fullPath, flags);
}

bool AFileImage::Open(std::string_view fullPath, unsigned int flags)
{
    if (m_isOpen)
        Close();
    ...
    {
        AFERRLOG("AFileImage::Open() Currently only supports read flag for file images");
        return false;
    }
...
bool AFileImage::Init(std::string_view fullPath)
{
    m_fileName = fullPath;
    m_relativeName = AFileMod::GetRelativePath(fullPath);
    ...
    if (g_pAFilePackage)
    {
        ...
        if (g_pAFilePackage->GetFileEntry(m_relativeName, entry))
        {
            ...
            {
                AFERRLOG("AFileImage::Init(), Error reading file [{}] from package!", m_relativeName);
                return false;
            }
    ...
    if (!file)
    {
        AFERRLOG("AFileImage::Init() Can't open file [{}] to create image in memory", fullPath);
        return false;
    }
    ...
    if (fileSize == 0)
    {
        AFERRLOG("AFileImage::Init() The file [{}] is zero length!", fullPath);
        return false;
    }
    ...
    {
        AFERRLOG("AFileImage::Init() Failed to read entire file [{}]", fullPath);
        return false;
    }
